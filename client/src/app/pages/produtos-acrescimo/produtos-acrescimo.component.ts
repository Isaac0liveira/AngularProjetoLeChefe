...
@Component({
  selector: 'app-produtos-acrescimo',
  templateUrl: './produtos-acrescimo.component.html',
  styleUrls: ['../produtos-bebidas/produtos-bebidas.component.scss']
})
export class ProdutosAcrescimoComponent implements OnInit {
  carrinho;
  produto;
  adm;
  contagem: number = 0;
  constructor(private get: SalvarService, private navegador: Router, private snackbar: MatSnackBar, private spinner: SpinnerService, private root: AppComponent, private dialog: MatDialog) {
    ...
    if(localStorage.getItem("superusuario") != null){
      ...
      this.snackbar.open("Acesso Não Autorizado!", 'x', {
        duration: 2000,
  ...
  subsDialogAdd(): Observable<any> {
    const dialogRef = this.dialog.open(AddProdutosComponent, {
      data: {tipo: "acrescimo", subTipo: false}
    });
    return dialogRef.afterClosed();
  }
