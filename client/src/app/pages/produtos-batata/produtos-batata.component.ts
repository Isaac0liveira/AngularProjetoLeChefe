...
@Component({
  selector: 'app-produtos-batata',
  templateUrl: './produtos-batata.component.html',
  styleUrls: ['../produtos-bebidas/produtos-bebidas.component.scss']
})
export class ProdutosBatataComponent implements OnInit {
  carrinho;
  produto;
  adm;
  contagem: number = 0;
  ...
  subsDialogAdd(): Observable<any> {
    const dialogRef = this.dialog.open(AddProdutosComponent, {
      data: {tipo: "batata", subTipo: false}
    });
    return dialogRef.afterClosed();
  }
  ...
  getProduto() {
    if (sessionStorage.getItem('produtos000') != undefined && sessionStorage.getItem('produtos000') != null ) {
      ...
      let produtoFiltro = entityInstance.filter(e => e.tipo == "batata")
      //@ts-ignore
