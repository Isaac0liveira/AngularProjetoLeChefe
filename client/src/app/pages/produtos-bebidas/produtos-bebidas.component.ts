...
@Component({
  selector: 'app-produtos-bebidas',
  templateUrl: './produtos-bebidas.component.html',
  styleUrls: ['./produtos-bebidas.component.scss']
})
export class ProdutosBebidasComponent implements OnInit {
  carrinho;
  produto;
  adm;
  contagem: number = 0;
  ...
  subsDialogAdd(): Observable<any> {
    const dialogRef = this.dialog.open(AddProdutosComponent, {
      data: {tipo: "bebidas", subTipo: false}
    });
    return dialogRef.afterClosed();
  }
  ...
  getProduto() {
    if (sessionStorage.getItem('produtos000') != undefined && sessionStorage.getItem('produtos000') != null ) {
      ...
      let produtoFiltro = entityInstance.filter(e => e.tipo == "bebidas")
      //@ts-ignore
