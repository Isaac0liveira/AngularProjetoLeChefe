...
@Component({
  selector: 'app-produtos-pizzas',
  templateUrl: './produtos-pizzas.component.html',
  styleUrls: ['../produtos-bebidas/produtos-bebidas.component.scss']
})
export class ProdutosPizzasComponent implements OnInit {
  carrinho;
  produto;
  adm;
  contagem: number = 0
  constructor(private get: SalvarService, private root: AppComponent, private spinner: SpinnerService, private dialog: MatDialog, private snackbar: MatSnackBar) {
  ...
  subsDialogAdd(): Observable<any> {
    const dialogRef = this.dialog.open(AddProdutosComponent, {
      data: {tipo: "pizzas", subTipo: false}
    });
    return dialogRef.afterClosed();
  }
  ...
  getProduto() {
    if (sessionStorage.getItem('produtos000') != undefined && sessionStorage.getItem('produtos000') != null ) {
      ...
      let produtoFiltro = entityInstance.filter(e => e.tipo == "pizzas")
      //@ts-ignore
