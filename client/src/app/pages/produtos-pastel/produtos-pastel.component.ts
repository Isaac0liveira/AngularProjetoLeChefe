...
@Component({
  selector: 'app-produtos-pastel',
  templateUrl: './produtos-pastel.component.html',
  styleUrls: ['../produtos-bebidas/produtos-bebidas.component.scss']
})
export class ProdutosPastelComponent implements OnInit {
  carrinho;
  produto;
  adm;
  contagem: number = 0;
  ...
  submit(quantidade: number, id: number){
  }
  ...
  subsDialogAdd(): Observable<any> {
    const dialogRef = this.dialog.open(AddProdutosComponent, {
      data: {tipo: "pastel", subTipo: false}
    });
    return dialogRef.afterClosed();
  }
  ...
  getProduto() {
    if (sessionStorage.getItem('produtos000') != undefined && sessionStorage.getItem('produtos000') != null ) {
      ...
      let produtoFiltro = entityInstance.filter(e => e.tipo == "pastel")
      //@ts-ignore
